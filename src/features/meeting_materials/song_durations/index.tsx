...
/**
 * Cuánto dura cada cántico.
 *
 * El cancionero `.jwpub` no lo trae —163 vídeos y ninguna duración—, pero
 * jw.org sí, y en una sola petición vienen todos. Sirve para una cosa muy
 ...
 * esperar —sale un cántico nuevo— y, sobre todo, para poder VER qué hay: sin
...
const SongDurations = () => {
  ...
  const descripcion = guardadas
    ? `${total} cánticos, desde jw.org el ${fechaCorta(guardadas.fetchedAt)}. Se actualizan solas una vez al mes; toca aquí para hacerlo ahora.`
  ...
  const handleClick = async () => {
    ...
    if (!nuevas) {
      displaySnackNotification({
        ...
        message:
          'jw.org no ha contestado. Comprueba la conexión y vuelve a intentarlo.',
        severity: 'error',
    ...
    displaySnackNotification({
      ...
      message: `${Object.keys(nuevas.seconds).length} cánticos, con lo que dura cada uno.`,
      severity: 'success',
  ...
  return (
    <ImportRow
      ...
      titulo="Duración de los cánticos"
