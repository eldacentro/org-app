  ...
  useEffect(() => {
    ...
    if (unreadNotices.length === 0) {
      setNotifications((prev) =>
        prev.filter((record) => !record.id.startsWith('territory-assigned-'))
      );
      return;
    }
    ...
    setNotifications((prev) => {
      ...
      let newValue = prev.filter(
        (record) => !record.id.startsWith('territory-assigned-')
      );

      // Crear una notificación por cada aviso
      const newNotifications: TerritoryAssignedNotificationType[] =
