...
const useSpeakersImportExport = () => {
  ...
  const exportCSV = useCallback(() => {
      ...
      .map((speaker) => {
        ...
        return {
          ...
          'Congregation Address':
            cong?.cong_data.cong_location.address.value || '',
          'Weekend Meeting Day':
            cong?.cong_data.weekend_meeting.weekday.value !== undefined
              ? String(cong.cong_data.weekend_meeting.weekday.value)
              : '',
          'Weekend Meeting Time':
            cong?.cong_data.weekend_meeting.time.value || '',
          'PTCoordinator Name':
            cong?.cong_data.public_talk_coordinator.name.value || '',
          'PTCoordinator Phone':
            cong?.cong_data.public_talk_coordinator.phone.value || '',
          'PTCoordinator Email':
            cong?.cong_data.public_talk_coordinator.email.value || '',
    ...
    link.setAttribute(
      'download',
      `speakers_export_${new Date().toISOString().split('T')[0]}.csv`
    );
    link.click();
