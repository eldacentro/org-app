...
const GoogleDriveTab = () => {
  ...
  const isAutoEnabled =
    backupAutomatic.google_drive_auto_enabled?.value || false;
    ...
    try {
      ...
      if (success) {
        displaySnackNotification({
          ...
          message:
            'La copia de seguridad ha sido guardada en Elda Centro App Backups.',
        });
      } else {
        displaySnackNotification({
          severity: 'error',
          ...
          message:
            'No se pudo completar la subida de datos a Google Drive. Asegúrese de que su sesión de Drive no haya expirado.',
        });
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <Stack spacing="24px" sx={{ mt: 1 }}>
      <Typography className="body-regular" color="var(--grey-400)">
        Resguarda copias de seguridad versión diaria cifrada directamente en tu
        Google Drive personal de forma automática.
      </Typography>
      ...
      <Box
        sx={{
          ...
          bgcolor: isConnected
            ? isTokenExpired
              ? 'color-mix(in srgb, var(--orange-main) 3%, transparent)'
              : 'var(--accent-100)'
            : 'var(--accent-100)',
        }}
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <Box
            sx={{
              ...
              bgcolor: isConnected
                ? isTokenExpired
                  ? 'color-mix(in srgb, var(--orange-main) 10%, transparent)'
                  : 'rgba(76, 175, 80, 0.1)'
          ...
          {isConnected ? (
            <Stack direction="row" spacing={1} alignItems="center">
              {isTokenExpired && (
                ...
                >
                  {isConnecting ? (
                    <IconLoading width={20} color="inherit" />
                  ) : (
                    'Reautorizar'
                  )}
                </Button>
      ...
      {isConnected && isTokenExpired && (
        <Box
          sx={{
            ...
            borderRadius: 'var(--shape-sm)',
            bgcolor: 'color-mix(in srgb, var(--orange-main) 4%, transparent)',
          }}
        >
          <Typography
            className="h4"
          ...
          <Typography className="body-regular" color="var(--grey-400)">
            Por políticas de seguridad de Google, la sesión local de acceso a
            archivos caduca después de 1 hora. Las copias de seguridad
            automáticas en Drive están pausadas en este dispositivo. Toca el
            botón <strong>Reautorizar</strong> a la derecha para reactivarlas.
          </Typography>
        </Box>
      )}
      ...
      {isConnected ? (
          ...
          >
            <Stack>
              <Typography className="h4">
                Subir Copia a la Nube Ahora
              </Typography>
              <Typography
                className="label-small-regular"
                color="var(--grey-400)"
              >
                Sincroniza y guarda el estado actual inmediatamente en tu Google
                Drive.
              </Typography>
            </Stack>
            <Button
        ...
        >
          ...
          <Typography
            ...
            sx={{ mb: 2 }}
          >
            1. **Copias en la Nube Automatizadas:** Cada día que uses la
            aplicación, se guardará una copia histórica rotativa de tus datos.
          </Typography>
          <Typography
            className="body-regular"
            color="var(--grey-400)"
            sx={{ mb: 2 }}
          >
            2. **Alcance de Privacidad Seguro (`drive.file`):** La aplicación
            solo tiene permiso para ver y modificar los archivos creados por
            ella misma. Tus fotos, documentos o carpetas privadas de Drive están
            totalmente fuera del alcance de la aplicación.
          </Typography>
          <Typography className="body-regular" color="var(--grey-400)">
            3. **Carpeta Identificable:** Todos los archivos se guardan
            organizados de manera centralizada en una carpeta exclusiva llamada
            **`Elda Centro App Backups`**.
          </Typography>
        </Box>
      )}
    </Stack>
