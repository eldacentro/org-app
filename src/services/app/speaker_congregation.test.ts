...
describe('qué se sella y qué no', () => {
  ...
  it('rellena la congregación que falta', () => {
    ...
    expect(stampSpeakerCongregation(semana, mapa)).toBe(true);
  ...
  it('NO pisa una congregación ya escrita', () => {
    ...
    expect(stampSpeakerCongregation(semana, mapa)).toBe(false);
    expect(semana.weekend_meeting.speaker.part_1[0].congregation).toBe(
  ...
  it('una semana sin orador no revienta', () => {
    expect(stampSpeakerCongregation(week({}), mapa)).toBe(false);
    expect(stampSpeakerCongregation({} as SchedWeekType, mapa)).toBe(false);
  });
