  ...
  const outingsSlotsInMonth = useMemo(() => {
    ...
    while (date.getMonth() === selectedMonth) {
      ...
      if (dayOfWeek === 2 || dayOfWeek === 3 || dayOfWeek === 4) {
        ...
        slots.push({
          date: new Date(date),
          slotType: `${dayLabel}_morning`,
          time: defaultHours[`${dayLabel}_morning` as keyof typeof defaultHours] || '10:00',
          slotId: `${formatToDbDate(date)}_morning`,
        });
        slots.push({
          date: new Date(date),
          slotType: `${dayLabel}_afternoon`,
          time: defaultHours[`${dayLabel}_afternoon` as keyof typeof defaultHours] || '17:00',
            ...
            >
              ...
              {settingsSubTab === 0 && (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  ...
                  <Box sx={{ display: 'flex', gap: '12px', maxWidth: '500px' }}>
                    <TextField
                    ...
                    {settings?.locations?.map((loc) => (
                      ...
                      >
                        ...
                        {settings.locations && settings.locations.length > 1 && (
                          <IconButton onClick={() => handleDeleteLocation(loc)} color="error" size="small">
                            <IconDelete />
                          </IconButton>
                        )}
                      </Card>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  ...
                  <Grid container spacing={3} sx={{ maxWidth: '600px' }}>
                    {[
                      ...
                      <Grid size={{ mobile: 12, tablet: 6 }} key={item.key}>
                        <TextField
                          label={item.label}
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: '20px', mt: '8px' }}>
          ...
          <FormControlLabel
            ...
            label={
              <Typography style={{ fontWeight: '600', color: editCancelled ? 'var(--error-dark)' : 'var(--black)' }}>
                {editCancelled ? 'Salida Cancelada' : 'Salida Activa'}
              </Typography>
            }
          />
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                >
                  ...
                  {sortedBrothersForSlot.recommended.length > 0 && (
                    <Box sx={{ paddingLeft: '8px', borderLeft: '3px solid var(--accent-main)', my: '4px' }}>
                      <Typography style={{ fontSize: '12px', fontWeight: '700', color: 'var(--accent-main)', textTransform: 'uppercase', padding: '4px 8px' }}>
                        Recomendados (Disponibles hoy)
                      </Typography>
                      {sortedBrothersForSlot.recommended.map((bro) => (
                        <MenuItem key={bro.person_uid} value={bro.person_uid} sx={{ pl: '16px' }}>
                          {`${bro.person_data.person_firstname.value} ${bro.person_data.person_lastname.value}`}
                        </MenuItem>
                      ))}
                    </Box>
                  ...
                  {sortedBrothersForSlot.others.length > 0 && (
                    <Box sx={{ paddingLeft: '8px', borderLeft: '3px solid var(--grey-400)', my: '4px' }}>
                      <Typography style={{ fontSize: '12px', fontWeight: '700', color: 'var(--grey-600)', textTransform: 'uppercase', padding: '4px 8px' }}>
                        Otros hermanos
                      </Typography>
